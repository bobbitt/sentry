...
5.0
date	96.08.14.09.27.01;	author MIKE;	state Exp;
branches;
next	;


desc
@Sentry System Maintenance Menu
@
...
/* Don't know waht this was meant to do... It's definitely wrong though...
      strcpy(cv,lo);
      strcat(cv,"\\");
      strcat(cv,cp);
      strcat(cv,pa);
...
short int x,y;
    ...
    else
    {
      ...
      else
      {
        if (!exffwd(1))
        {
          ...
          if (strcmp(lo,ver)<0)
          {
            cprintf("\r\nThis file is protected by an earlier version of %s.",SENTRY);
  ...
  if (!bk)
  {
    ...
    lo[strlen(lo)+1]=0;
#if !USER
    lo[strlen(lo)]='X';  /* Set X-ray mode */
#else
    lo[strlen(lo)]='Z';  /* Set Zulu mode */
#endif
    fprintf(npas,"%s\r\n",lo);
    fprintf(npas,"%s\r\n",ex);
...
long loc[MAXUSER],yzl,zzl;
  ...
  while (x && !bk)
  {
    ...
    fclose(pass);
//***** Check to see it's really a .com file
  }
  if (!bk)
  {
    if ((pass=fopen(na,"rb"))==NULL)
...
/**** What about non-windowed mode?? */
  ...
  strcpy(txt[13],"[ ] Accounts that have expired (should be deleted)");
  strcpy(txt[14],"[ ] # of failed login attempts is exceeded");
  strcpy(txt[15],"[ ] SuperUser accounts with a max inv log setting");
  strcpy(txt[16],"[ ] Normal accounts with no max inv log setting");
  strcpy(txt[17],"[ ] Accounts with no expiry date");
